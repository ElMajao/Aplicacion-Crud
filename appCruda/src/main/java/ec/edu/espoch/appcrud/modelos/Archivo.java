...
public class Archivo {
    ...
    private File obtenerArchivo() {
        try {
            URL url = getClass().getClassLoader().getResource("ec/edu/espoch/appcrud/modelos/" + nombre);
            if (url != null) {
                return new File(url.toURI());
            } else {
                // Si el archivo no existe, se crea uno nuevo en el mismo directorio
                File nuevoArchivo = new File(getClass().getClassLoader().getResource("ec/edu/espoch/appcrud/modelos/").toURI().getPath() + nombre);
